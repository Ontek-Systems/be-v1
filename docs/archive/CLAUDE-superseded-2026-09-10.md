...
- **Do not use pure black** for text, backgrounds or UI. Use `primary-navy` wherever a near-black is needed.
- **Image scrims are always black, and always run bottom to top** (`bg-gradient-to-t from-black/... to-transparent`). Agreed on 10 Sept 2026, replacing the earlier per component exceptions. Navy and sky scrims read as a blue wash over the photograph; black reads as shadow and lets the image keep its colour. No top down, side on, or brand coloured gradients over images anywhere on the public site, and no top scrim behind the header.
...
### 13. Copywriting: always read COPYWRITING.md first (STRICT)
- `COPYWRITING.md` at the repo root is the single source of truth for every user-facing word on the site.
- **Read `COPYWRITING.md` in full before writing, rewriting, or reviewing ANY copy.** That covers headings, body text, eyebrows, button labels, form labels and placeholders, alt text, meta titles and descriptions, error and success messages, and email templates. It applies whenever a request touches wording, however small, and whether or not the file is mentioned.
- Apply the correct voice (§2), the page brief (§3), and the house style (§6). Run every block through the Kill List (§8) and the Checklist (§10) as you write.
- **Then just make the changes.** Do not write an audit of the existing copy, do not list violations back, and do not ask for approval on wording first. Read, write, apply, then report briefly on what changed and anything genuinely blocked.
- Never invent facts about the business. Use §9 (facts table). Anything marked TBC must be confirmed with the client, not guessed.
- Hard bans, no exceptions: em dashes, en dashes, hyphens in prose, exclamation marks, alliterative rule-of-three headlines, and every term on the Kill List.
- If copy needs to break a rule in `COPYWRITING.md`, update `COPYWRITING.md` first and say why. The file leads, the site follows.
