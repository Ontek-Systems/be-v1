...
export function VisitsChart({ points, granularity, range, onRangeChange, isLoading }: Readonly<VisitsChartProps>) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-primary-navy/20 bg-primary-navy/[0.035] p-5 shadow-lg shadow-primary-navy/10 sm:p-7">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-24 -right-24 h-56 w-56 rounded-full bg-primary-sky/20 blur-3xl"
      />

      <div className="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary-sky">Site visits</p>
          <div className="mt-1 flex items-baseline gap-2">
            <span className="text-3xl font-semibold text-primary-navy sm:text-4xl">
              {totalVisits.toLocaleString()}
            </span>
            <span className="text-sm text-primary-sky">
              visits in the last {range === "7d" ? "7 days" : range === "30d" ? "30 days" : "90 days"}
            </span>
          </div>
        </div>

        <div className="flex items-center gap-1 self-start rounded-lg border border-primary-navy/20 bg-primary-navy/5 p-1">
          {RANGE_OPTIONS.map((option) => (
            <button
              key={option.value}
              type="button"
              onClick={() => onRangeChange(option.value)}
              className={`cursor-pointer rounded-md px-3 py-1.5 text-xs font-semibold tracking-wide transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-navy ${
                range === option.value
                  ? "bg-primary-navy text-white shadow-sm"
                  : "text-primary-sky hover:text-primary-navy"
              }`}
            >
              {option.label}
            </button>
      ...
      <div className={`relative mt-4 transition-opacity duration-300 ${isLoading ? "opacity-50" : "opacity-100"}`}>
        {points.length === 0 ? (
          <div className="flex h-[260px] items-center justify-center">
            <p className="text-sm text-primary-sky">No visit data yet</p>
          </div>
        ) : (
          <>
            <svg
            ...
            >
              <defs>
                <linearGradient id="visits-area-fill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#274957" stopOpacity="0.16" />
                  <stop offset="100%" stopColor="#274957" stopOpacity="0" />
                </linearGradient>
              </defs>
              ...
              <path d={linePath} fill="none" stroke="#274957" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
              ...
              {coords.map((c, i) =>
                i % tickStep === 0 ? (
                  <text
                    ...
                    y={CHART_HEIGHT - 8}
                    textAnchor="middle"
                    className="fill-primary-sky text-[10px]"
                  >
              ...
              {hovered && (
                <>
                  <line
                    ...
                    strokeWidth="1"
                  />
                  <circle cx={hovered.x} cy={hovered.y} r="4" fill="#274957" stroke="#fff" strokeWidth="2" />
                </>
              )}
            </svg>
            ...
            {hovered && (
              <div
                className="pointer-events-none absolute top-2 -translate-x-1/2 rounded-lg bg-primary-navy px-3 py-2 text-white shadow-xl shadow-primary-navy/40"
                style={{ left: `${(hovered.x / width) * 100}%` }}
              >
                <p className="text-[10px] uppercase tracking-[0.15em] text-primary-sky">
      ...
      {showTable && (
        <div className="relative mt-3 max-h-64 overflow-auto rounded-lg border border-primary-navy/20">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {points.map((point) => (
                <tr key={point.timestamp} className="border-b border-primary-navy/18 last:border-b-0">
                  <td className="px-3 py-2 text-primary-sky">{formatLabel(point.timestamp, granularity)}</td>
                  <td className="px-3 py-2 font-medium text-primary-navy">{point.count.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
